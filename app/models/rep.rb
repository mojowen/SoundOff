class Rep < ActiveRecord::Base
	...
	def self.search search
		all(conditions: [
			('LOWER(first_name) || \' \' || LOWER(last_name) LIKE :search OR ' \
			 'LOWER(state_name) LIKE :search OR ' \
			 'LOWER(twitter_screen_name) LIKE :search'),
			{ search: "%#{search.downcase}%" }
		])
	end

	def self.lookup(list)
		where(
			'LOWER(twitter_screen_name) IN(?) OR LOWER(bioguide_id) IN(?)',
			list,
			list
		)
	end
